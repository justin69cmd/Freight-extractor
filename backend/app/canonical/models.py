...
class DocumentChunk(Base, TimestampMixin):
    ...
    ref_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), index=True)  # source row id
...
class ReviewTask(Base, TimestampMixin):
    ...
    item_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), index=True)
